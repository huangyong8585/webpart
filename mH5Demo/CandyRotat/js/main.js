...
var card = (function(total,cardnum){
    ...
    var MOUSE_UP = 'ontouchend' in document ? 'touchend' : 'mouseup';
    var MOUSE_MOVE = 'ontouchmove' in document ? 'touchmove' : 'mousemove';*/
	var MOUSE_ClICK = 'ontouchstart' in document ? 'touchstart' : 'click';
    ...
    init = function(){
        ...
        $$('startgame').onclick = function(){
            tips('hide');
			 $$('gameplane').style.visibility="visible";
            start();
        }
		$$('againgame').onclick = function(){
			document.title="万圣糖果翻翻乐 HAPPY哈喽喂 天才宝贝";
			disp('gameplane', 'show');
			showPOP()
			level = 1;
			score = 0;			
    ...
    show = function(){
        ...
        for(var i=0; i<leveldata.length; i++){
            ...
            cardhtml += '<div class="list flip out"><img src="image/candy' + leveldata[i]['cardno'] + '.png"></div>';
            cardhtml += '<div class="list flip"><img src="image/candybg.png"></div>';
            cardhtml += '</div>';
            cardhtml += '</div>';
    ...
    set_event = function(){
        ...
        for(var i=0,count=o.length; i<count; i++){
            if(getClass(o[i])=='card viewport-flip'){
                o[i].addEventListener(MOUSE_ClICK,function(){
                    turn(this.id);
                },false);
            }
        }
    ...
    process = function(){
        ...
        var et = setInterval(function(){
            ...
            if(curtime==0){
                ...
                is_over = 1;
                //message('fail', start);
				message('fail', function(){showPOP(score>=100?'还不错哦':'再接再厉')});
				document.title="我在天才宝贝万圣糖果翻翻乐中获得了"+score+"分，你也来试试吧!!";
            }
    ...
    message = function(type, callback){
        ...
        //setPosition(message, 'left', 30);
        //setPosition(message, 'top', 50);
    ...
    clear = function(){
        ...
        //setHtml('gameplane','');
        //message('clear',init);
		message('clear',function(){showPOP('厉害！！');});
    }
    ...
    reset = function(){
        ...
        setHtml('level', level);setHtml('score', score);
        disp('process', 'show');
		disp('scorebox', 'show');
        setHtml('livetime', '');
        setHtml('gameplane', '');
